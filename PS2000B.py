...
class Mypower:
    ...
    def connect(self, mcom):
        DEVICE = "COM{}".format(mcom) if platform.system() == "Windows" else "/dev/ttyACM0"

        # 连接设备
        print("连接到设备 %s..." % DEVICE)
        try:
            self.device = PS2000B(DEVICE)
...
if __name__ == '__main__':
    c: int = int(input("输入端口："))
    powe = Mypower()
    powe.connect(c)
    powe.open_power()
    powe.control_power(13.5, 3)
    time.sleep(2)
